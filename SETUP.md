...
`supabase/seed-3de-leerjaar.sql` voegt in één keer toe (uitvoeren na het vernieuwde `schema.sql`): drie **Cirkelrekenen**-oefeningen (optellen, aftrekken, gemengd – de sommen worden elke keer opnieuw willekeurig gemaakt), en vijf meerkeuzequizzen: *Getallen tot 100*, *Tafels van 2, 5 en 10*, *Taal: letters en woorden*, *Klok kijken* en *België: provincies en gewesten*. De brontekst van de eerste vier staat in `supabase/quizzen/*.txt` (in het importformaat, dus ook bruikbaar als voorbeeld). Vak-indeling: Cirkelrekenen/Getallen tot 100/Tafels → **Rekenen**, Taal: letters en woorden → **Taal**, Klok kijken → **Varia**, België → **Wereldoriëntatie**.
...
Op het dashboard staan **enkel de vaknamen** zichtbaar; klik op een vak om de quizzen erin te tonen (en nog eens klikken om weer dicht te klappen). Zo blijft de startpagina overzichtelijk, ook met veel quizzen.

**Extra quiz: België (provincies en gewesten).** Een meerkeuzequiz voor het 2de/3de leerjaar met 15 vragen over de gewesten, talen, hoofdsteden en provincies van België (o.a. welke provincie het kleinst/grootst is, welke rivieren samenkomen in Namen, enz.). Zit in `supabase/seed-3de-leerjaar.sql` onder het vak **Wereldoriëntatie**. Er is bewust géén klikbare landkaart aan toegevoegd: een kaart die niet de echte vorm van België heeft zou kinderen net een verkeerd beeld geven, dus dit blijft een gewone meerkeuzequiz met feitjes per provincie. Demo zonder login: `leerpret.be/quiz?demo=belgie`.
